-- Simplified Database Schema for DFS Portal
-- Core tables only without Supabase-specific features
...
CREATE TABLE user_profiles (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID, -- Removed reference to auth.users for compatibility
...
CREATE TABLE alert_settings (
    ...
    created_by UUID, -- Removed reference to auth.users for compatibility
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE alert_history (
    ...
    resolved_by UUID, -- Removed reference to auth.users for compatibility
